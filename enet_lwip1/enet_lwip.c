...
echo_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
  ...
  else if(es->state == ES_ACCEPTED)
  {
    ...
    UARTprintf("\r\n Accept Successful, payload size = %d\r\n",p->tot_len);
    es->p = p;
    //echo_send(tpcb, es);
  ...
  else if (es->state == ES_RECEIVED)
  {
        ...
        else
        {
          ...
          p->payload = logmsg;
          UARTprintf("\r\n[SocketThread] source ID: %d \n", logmsg->sourceId);
          UARTprintf("\r\n[SocketThread] Log Level: %d \n", logmsg->level);
          UARTprintf("\r\n[SocketThread] Payload: %s   \n", logmsg->payload);
          UARTprintf("\r\n[SocketThread] Timestamp: %s \n", ctime(&logmsg->timestamp));

          echo_send(tpcb, es);

          /*actual data from TIVA */
          logmsg->sourceId= LOGGER_TASK;
          logmsg->requestID= LOG_DATA;
          logmsg->data = 25.0;
          logmsg->level = INFO;
          strcpy(logmsg->payload,"Data from Temp thread");
          logmsg->timestamp = time(NULL);
          p->payload = logmsg;
